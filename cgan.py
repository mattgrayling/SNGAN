...
class CGAN:
    def __init__(self, latent_dims=100, dlr=0.00001, glr=0.00001, epsilon=0.1, beta1=0.9, beta2=0.999, gen_activation='sigmoid', device='gpu:0',
                 labels='hard', GP=True, data_type='sim', z_lim=None, batch_norm=False, mode='template',
        ...
        if self.data_type == 'real':
            ...
            self.name = f'CGAN_DES_sim_CCSNe_{self.mode}_dlr{self.dlr}_glr{self.glr}_epsilon{self.epsilon}_' \
                        f'ld{self.latent_dims}_labels{self.labels}_GP{self.GP}_zlim{self.z_lim}_bn{self.batch_norm}' \
                        f'_N{self.units}_beta1{self.beta2}_beta2{self.beta2}'
        self.root = os.path.join('Data', 'Models', self.mode, self.name)
        if not os.path.exists(self.root):
            os.mkdir(self.root)
        if self.data_type == 'real':
            self.dataset_name = f'CGAN_DES_real_GP{self.GP}__{self.generator_type}'
